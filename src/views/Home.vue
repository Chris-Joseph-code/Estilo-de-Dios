              ...
              <v-layout align-center justify-center>
                <v-flex xs12>
                  <v-toolbar dark color="primary darken-1">
                    ...
                    <v-icon style="position: absolute; left: 90%" @click="chat = !chat"
                      >close</v-icon
                    >
                  </v-toolbar>
                  <v-card-text color="primary">
                    <v-list ref="chat" id="logs">
                      ...
                      <template v-for="(item, index) in logs">
                        <v-subheader
                          :class="item.cm"
                          style="margin-block: 3px; border-radius: 20px; height: auto"
                          v-if="item"
                          :key="index"
                        >
                          <small
  ...
  methods: {
    ...
    async submit() {
      if (this.msg !== "") {
        ...
        db.collection("chat").add({ m: this.msg, u: this.user, cm: this.colorMine });
        this.logs.push({ m: this.msg, u: this.user, cm: this.colorMine });
        this.msg = "";
      }
    },
  },
